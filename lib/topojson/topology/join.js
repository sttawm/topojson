...
module.exports = function(topology) {
  var coordinates = topology.coordinates,
      ...
      ringGeoms = topology.ringGeoms,
      lineGeoms = topology.lineGeoms,
      indexes = index(),
      ...
      options = topology.options ||
          { ignoreSelfIntersections: true,
            ignoreRingGeometryIntersections: false,
            ignoreLineGeometryIntersections: false
          },
      ignoreSelfIntersections = options.ignoreSelfIntersections,
      ignoreRingGeometryIntersections = options.ignoreRingGeometryIntersections,
      ignoreLineGeometryIntersections = options.ignoreLineGeometryIntersections;

  clear(leftByIndex);
  clear(rightByIndex);
  if (ignoreSelfIntersections) clear(visitedByIndex);

  for (var i = 0, n = lines.length, g = 1; i < n; ++i) {
    var line = lines[i],
        lineStart = line[0],
        lineEnd = line[1],
        previousIndex,
    ...
    while (++lineStart <= lineEnd) {
      sequence(lineGeoms[g], previousIndex = currentIndex, currentIndex = nextIndex, nextIndex = indexes[lineStart]);
    }
  ...
  for (var i = 0, n = rings.length, g = 1; i < n; ++i) {
    var ring = rings[i],
        ringStart = ring[0] + 1,
        ringEnd = ring[1],
    ...
    while (++ringStart <= ringEnd) {
      sequence(g, previousIndex = currentIndex, currentIndex = nextIndex, nextIndex = indexes[ringStart]);
    }

    // If the junction count increased as a result of processing this ring, it has a junction.
